...
export default function Sub9NoticeUpdateFormCom () {
    ...
    React.useEffect(()=>{
        ...
        if(selector.adminsignIn.adminsignInData.id!==''){
            setState({
                ...state,
    ...
    const onSubmitInsertForm=(e)=>{
        ...
        if(state.유형===''){
            ...
            axios({
                url:'https://sieun.co.kr/innisfree/innisfree_notice_table_update.php',
                method: 'POST',
                data: formData
            })
            .then((res)=>{               
                if(res.status===200){   
                    if(res.data===1){                        
                        ...
                        messageModalMethod('공지 수정에 성공하였습니다.');
                        navigate('/sub9Notice');
                    }                 
                    else{
                        alert('공지 수정에 실패하였습니다. 확인 후 다시 시도해주세요.');
                    }
